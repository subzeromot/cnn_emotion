...
class KerasFaceAnalyzerBase():
    ...
    def _build_train_model(self, num_filters, num_blocks, input_shape, num_classes):
        ...
        # # conv layer with Relu
        # model.add(Conv2D(10, (3,3), input_shape=input_shape, activation='relu', padding='same'))
        ...
        # # conv layer with Relu
        # model.add(Conv2D(1, (1,1), activation='tanh'))
    ...
    def _build_data_generator(self, output_dir):
        ...
        data_generator = keras.preprocessing.image.ImageDataGenerator(horizontal_flip=True,
    ...
    def train(self, output_dir, model_fname, epochs=5):
        ...
        model = self._build_train_model(num_filters, num_blocks, X_train[0].shape, Y_train[0].shape[0])
        ...
        history = model.fit(X_train, Y_train, verbose=1, shuffle=True, epochs=epochs, validation_split=.2, callbacks=[model_cb, history_cb], class_weight=class_weight_dict)

        # train_data = data_generator.flow(X_train, Y_train, batch_size=32, subset='training')
        # val_data = data_generator.flow(X_train, Y_train, batch_size=32, subset='validation')
        # history = model.fit_generator(train_data, validation_data=val_data, verbose=1, shuffle=True, epochs=epochs, callbacks=[model_cb, history_cb], class_weight=class_weight_dict)
...
class MyKerasFaceAnalyzer(KerasFaceAnalyzerBase):
    def _build_train_model(self):
        ...
        model = keras.models.Sequential()
        model.add(keras.layers.Dense(len(self._class_labels), activation='softmax'))
...
def main(argv):
    ...
    analyzer = KerasFaceAnalyzerBase('emotion')
