...
class KerasFaceAnalyzerBase():
    ...
    def _alex_net(self, num_filters, num_blocks, input_shape, num_classes):
        model = keras.models.Sequential([
                    keras.layers.Conv2D(filters=96, kernel_size=(5), strides=2, input_shape=input_shape, activation='relu'),
                    keras.layers.MaxPool2D(pool_size=3, strides=2, padding='same'),
                    keras.layers.Conv2D(filters=256, kernel_size=5, activation='relu', padding='same'),
                    keras.layers.MaxPool2D(pool_size=3, strides=2),
                    keras.layers.Conv2D(filters=384, kernel_size=3, activation='relu', padding='same'),
                    keras.layers.Conv2D(filters=384, kernel_size=3, activation='relu', padding='same'),
    ...
    def _vgg_block(self, num_convs, num_channels):
        ...
        for _ in range(num_convs):
            blk.add(keras.layers.Conv2D(num_channels, kernel_size=3, activation='relu', padding='same'))
        blk.add(keras.layers.MaxPool2D(pool_size=3, strides=2))
        return blk

    def _vgg_net(self, conv_arch, input_shape, num_classes):
        model = keras.models.Sequential()
        
        # Conv part
        model.add(keras.layers.Conv2D(filters=16, kernel_size=(5), strides=2, input_shape=input_shape, activation='relu'))
        for (num_convs, num_channels) in conv_arch:
            model.add(self._vgg_block(num_convs, num_channels))
    ...
    def _build_train_model(self, num_filters, num_blocks, input_shape, num_classes):
        ...
        ##### AlexNet
        model = self._alex_net(num_filters, num_blocks, input_shape, num_classes)

        ##### VGG
        # conv_arch = ((1, 64), (1, 128), (2, 256), (2, 512), (2, 512))
        # model = self._vgg_net(conv_arch, input_shape, num_classes)
