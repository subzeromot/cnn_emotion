...
class Shape(object):
    ...
    def display(self, background_img=None, fig=None, pointcolor='r.', textcolor='red', title=None):
        ...
        x_min, x_max = int(np.min(self.points[:,0])), int(np.max(self.points[:,0]))
        y_min, y_max = int(np.min(self.points[:,1])), int(np.max(self.points[:,1]))
        img_crop = background_img[y_min:y_max, x_min:x_max]
        ...
        if background_img is not None:
            fig.gca().imshow(img_crop, cmap='gray')
